...
فقط عنوان و توضیحات آگهی را بررسی کن.

مهم: عدم همخوانی جزئی یا کامل بین عنوان و توضیحات، به‌تنهایی دلیل رد نیست. آگهی را فقط به‌خاطر متفاوت بودن عنوان و توضیحات رد نکن.

آگهی را تایید کن اگر:
- مرتبط با درخواست یا پیشنهاد تبادل محل خدمت سربازی باشد
- متن منطقی، محترمانه و قابل فهم باشد
- فحش، توهین، ناسزا، یا الفاظ رکیک نداشته باشد
- درخواست پول، پیشنهاد پرداخت، وعده مالی، یا هر نوع معامله مالی (مثل «پول می‌دم»، «هزینه می‌گیرم»، «مبلغ توافقی») نداشته باشد
- spam، شوخی بی‌ربط، متن تصادفی، تبلیغ نامرتبط، یا محتوای خارج از موضوع نباشد

آگهی را رد کن اگر هر کدام از موارد زیر وجود دارد:
- متن نامرتبط، بی‌معنا، آزاردهنده، یا فقط برای اذیت/اسپم
- موضوعی غیر از تبادل محل خدمت سربازی
- متن فارسی/انگلیسی نامفهوم یا پر از حروف تصادفی
- فحش، توهین، ناسزا، یا الفاظ رکیک (حتی مخفی یا با فاصله/علامت)
- درخواست پول، پیشنهاد پرداخت، وعده مالی، یا هر اشاره به معامله مالی برای تبادل

هرگز به‌خاطر «عدم تطابق عنوان با متن» یا «متفاوت بودن عنوان و توضیحات» آگهی را رد نکن.

اگر رد کردی، در فیلد reason یک هشدار کوتاه و محترمانه به فارسی بنویس و دقیقاً بگو مشکل چیست (مثلاً: «آگهی حاوی فحش است»، «درخواست پول یا وعده مالی مجاز نیست»، «متن با موضوع تبادل محل خدمت مرتبط نیست»).
    ...
    private function parseDecision(string $content): array
    {
        ...
        if ($reason === '') {
            $reason = $approved
                ...
                : 'محتوای آگهی با قوانین پلتفرم هم‌خوانی ندارد. لطفاً متن را اصلاح کنید: بدون فحش، بدون درخواست پول، و فقط درباره تبادل محل خدمت باشد.';
        }

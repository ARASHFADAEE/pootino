<article class="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:shadow-md">
    <div class="mb-3 flex items-center justify-between text-xs text-slate-500">
        ...
        <span>{{ optional($ad->approved_at)->format('Y/m/d') }}</span>
    </div>
    ...
    <div class="mt-4 flex items-center justify-between border-t border-slate-100 pt-3 text-xs">
        ...
        <a href="{{ route('ads.show', $ad) }}" class="rounded-lg bg-[var(--color-military-700)] px-3 py-1.5 text-white">مشاهده</a>
    </div>
</article>
